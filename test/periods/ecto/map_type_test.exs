defmodule Periods.Ecto.MapTypeTest do
  ...
  describe "cast/1" do
    test "with a map returns ok and a Period" do
      assert MapType.cast(%{amount: 100, unit: :day}) == {:ok, %Periods.Period{amount: 100, unit: :day}}
    end
    ...
    test "with an invalid value returns error" do
      assert MapType.cast(1.23) == {:error, :amount_must_be_an_integer}
    end
  end

  describe "load/1" do
    test "load/1 map with integer amount" do
      assert MapType.load(%{"amount" => 100, "unit" => "hour"}) == {:ok, %Period{amount: 100, unit: :hour}}
    end
  ...
  describe "dump/1" do
    test "converts into proper map" do
      assert MapType.dump(%Period{amount: 100, unit: :week}) == {:ok, %{"amount" => 100, "unit" => "week"}}
    end
